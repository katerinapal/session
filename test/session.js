...
describe('session()', function () {
  it('should export constructors', function () {
    _assert2.default.equal(_typeof(_.session.Session), 'function');
    _assert2.default.equal(_typeof(_.session.Store), 'function');
    _assert2.default.equal(_typeof(_.session.MemoryStore), 'function');
  });
  ...
  it('should error without secret', function (done) {
    request(createServer({ secret: undefined })).get('/').expect(500, /secret.*required/, done);
  });
  ...
  it('should get secret from req.secret', function (done) {
    ...
    request(createServer(setup, { secret: undefined })).get('/').expect(200, '', done);
  });

  it('should create a new session', function (done) {
    var store = new _.session.MemoryStore();
  ...
  it('should create multiple sessions', function (done) {
    ...
    function check(err) {
      ...
      store.all(function (err, sess) {
        if (err) return done(err);
        _assert2.default.equal(Object.keys(sess).length, 2);
        done();
      });
    }
  ...
  it('should not save with bogus req.sessionID', function (done) {
    ...
    request(server).get('/').expect(shouldNotHaveHeader('Set-Cookie')).expect(200, function (err) {
      ...
      store.length(function (err, length) {
        if (err) return done(err);
        _assert2.default.equal(length, 0);
        done();
      });
    });
  });
    ...
    request(server).get('/').expect(shouldSetCookie('connect.sid')).expect(200, function (err, res) {
      ...
      setTimeout(function () {
        request(server).get('/').set('Cookie', cookie(res)).expect(shouldSetCookie('connect.sid')).expect(function (res) {
          _assert2.default.notEqual(originalExpires, expires(res));
        }).expect(200, done);
  ...
  describe('when response ended', function () {
    it('should have saved session', function (done) {
      ...
      request(server).get('/').expect(200, 'session saved', function (err) {
        if (err) return done(err);
    ...
    it('should have saved session even with multi-write', function (done) {
      ...
      request(server).get('/').expect(200, 'hello, world', function (err) {
        if (err) return done(err);
        _assert2.default.ok(saved);
        done();
      });
    ...
    it('should have saved session with updated cookie expiration', function (done) {
      ...
      request(server).get('/').expect(shouldSetCookie('connect.sid')).expect(200, function (err, res) {
        ...
        store.get(id, function (err, sess) {
          ...
          _assert2.default.equal(exp.toUTCString(), expires(res));
          setTimeout(function () {
            request(server).get('/').set('Cookie', cookie(res)).expect(200, function (err, res) {
              if (err) return done(err);
              store.get(id, function (err, sess) {
                if (err) return done(err);
                _assert2.default.equal(res.text, id);
                _assert2.default.ok(sess, 'session still in store');
                _assert2.default.notEqual(new Date(sess.cookie.expires).toUTCString(), exp.toUTCString(), 'session cookie expiration updated');
                done();
              });
            });
          }, 1000 - Date.now() % 1000 + 200);
  ...
  describe('when sid not in store', function () {
    ...
    it('should have a new sid', function (done) {
      ...
      request(server).get('/').expect(shouldSetCookie('connect.sid')).expect(200, 'session 1', function (err, res) {
        ...
        store.clear(function (err) {
          if (err) return done(err);
          request(server).get('/').set('Cookie', cookie(res)).expect(shouldSetCookie('connect.sid')).expect(200, 'session 2', function (err, res) {
            if (err) return done(err);
            _assert2.default.notEqual(sid(res), val);
            done();
          });
        });
      });
  ...
  describe('when sid not properly signed', function () {
    it('should generate new session', function (done) {
      ...
      request(server).get('/').expect(shouldSetCookie('sessid')).expect(200, 'session created', function (err, res) {
        ...
        var val = sid(res);
        _assert2.default.ok(val);
        request(server).get('/').set('Cookie', 'sessid=' + val).expect(shouldSetCookie('sessid')).expect(200, 'session created', done);
      });
    });
    ...
    it('should not exist in store', function (done) {
      ...
      request(server).get('/').expect(shouldSetCookie('connect.sid')).expect(200, 'session 1', function (err, res) {
        ...
        setTimeout(function () {
          store.all(function (err, sess) {
            if (err) return done(err);
            _assert2.default.equal(Object.keys(sess).length, 0);
            done();
          });
        }, 10);
      });
  ...
  describe('genid option', function () {
    ...
    it('should allow custom function', function (done) {
      ...
      request(createServer({ genid: genid })).get('/').expect(shouldSetCookieToValue('connect.sid', 's%3Aapple.D8Y%2BpkTAmeR0PobOhY4G97PRW%2Bj7bUnP%2F5m6%2FOn1MCU')).expect(200, done);
    });
    ...
    it('should encode unsafe chars', function (done) {
      ...
      request(createServer({ genid: genid })).get('/').expect(shouldSetCookieToValue('connect.sid', 's%3A%25.kzQ6x52kKVdF35Qh62AWk4ZekS28K5XYCXKa%2FOTZ01g')).expect(200, done);
    });
    ...
    it('should provide req argument', function (done) {
      ...
      request(createServer({ genid: genid })).get('/foo').expect(shouldSetCookieToValue('connect.sid', 's%3A%2Ffoo.paEKBtAHbV5s1IB8B2zPnzAgYmmnRPIqObW4VRYj%2FMQ')).expect(200, done);
    });
  });
  ...
  describe('key option', function () {
    ...
    it('should allow overriding', function (done) {
      request(createServer({ key: 'session_id' })).get('/').expect(shouldSetCookie('session_id')).expect(200, done);
    });
  });
  ...
  describe('resave option', function () {
    ...
    it('should pass session touch error', function (done) {
      ...
      server.on('error', function onerror(err) {
        ...
        _assert2.default.equal(err.message, 'boom!');
        cb();
      });
    ...
    describe('.regenerate()', function () {
      it('should destroy/replace the previous session', function (done) {
        ...
        request(server).get('/').expect(shouldSetCookie('connect.sid')).expect(200, function (err, res) {
          if (err) return done(err);
          var id = sid(res);
          request(server).get('/').set('Cookie', cookie(res)).expect(shouldSetCookie('connect.sid')).expect(200, 'false', function (err, res) {
            if (err) return done(err);
            _assert2.default.notEqual(sid(res), id);
            done();
          });
        });
      });
          ...
          store.get(id, function (err, sess) {
            ...
            setTimeout(function () {
              request(server).get('/').set('Cookie', cookie(res)).expect(200, function (err, res) {
                ...
                store.get(id, function (err, sess) {
                  if (err) return done(err);
                  _assert2.default.notEqual(new Date(sess.cookie.expires).getTime(), exp.getTime());
                  done();
                });
              });
            }, 100);
    ...
    describe('.cookie', function () {
      describe('.*', function () {
        ...
        it('should default to a browser-session length cookie', function (done) {
          request(createServer({ cookie: { path: '/admin' } })).get('/admin').expect(shouldSetCookieWithoutAttribute('connect.sid', 'Expires')).expect(200, done);
        });
      ...
      describe('.maxAge', function () {
        before(function (done) {
          ...
          request(ctx.server).get('/').end(function (err, res) {
...
function shouldSetCookie(name) {
  return function (res) {
    ...
    _assert2.default.equal(data.name, name, 'should set cookie ' + name);
  };
}
...
function shouldSetCookieToExpireIn(name, delta) {
  return function (res) {
    ...
    _assert2.default.equal(Date.parse(data.expires) - Date.parse(res.headers.date), delta, 'should set cookie ' + name + ' to expire in ' + delta + ' ms');
  };
}
...
function shouldSetCookieToValue(name, val) {
  return function (res) {
    ...
    _assert2.default.ok(header, 'should have a cookie header');
    _assert2.default.equal(data.name, name, 'should set cookie ' + name);
    _assert2.default.equal(data.value, val, 'should set cookie ' + name + ' to ' + val);
  };
}
...
function shouldSetCookieWithAttributeAndValue(name, attrib, value) {
  return function (res) {
    ...
    _assert2.default.equal(data.name, name, 'should set cookie ' + name);
    _assert2.default.ok(attrib.toLowerCase() in data, 'should set cookie with attribute ' + attrib);
    _assert2.default.equal(data[attrib.toLowerCase()], value, 'should set cookie with attribute ' + attrib + ' set to ' + value);
  };
}
...
function SyncStore() {
  _.session.Store.call(this);
  this.sessions = Object.create(null);
}

_util2.default.inherits(SyncStore, _.session.Store);
