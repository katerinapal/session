...
var session = _interopRequireWildcard(_);
...
describe('session()', function () {
  it('should export constructors', function () {
    _assert2.default.equal(_typeof(session.Session), 'function');
    _assert2.default.equal(_typeof(session.Store), 'function');
    _assert2.default.equal(_typeof(session.MemoryStore), 'function');
  });
  ...
  describe('when sid not in store', function () {
    ...
    it('should have a new sid', function (done) {
      ...
      (0, _supertest2.default)(server).get('/').expect(shouldSetCookie('connect.sid')).expect(200, 'session 1', function (err, res) {
        ...
        store.clear(function (err) {
          if (err) return done(err);
          (0, _supertest2.default)(server).get('/').set('Cookie', cookie(res)).expect(shouldSetCookie('connect.sid')).expect(200, 'session 2', function (err, res) {
            if (err) return done(err);
  ...
  describe('when sid not properly signed', function () {
    it('should generate new session', function (done) {
      ...
      (0, _supertest2.default)(server).get('/').expect(shouldSetCookie('sessid')).expect(200, 'session created', function (err, res) {
        ...
        var val = sid(res);
        _assert2.default.ok(val);
        (0, _supertest2.default)(server).get('/').set('Cookie', 'sessid=' + val).expect(shouldSetCookie('sessid')).expect(200, 'session created', done);
      });
    });
  ...
  describe('genid option', function () {
    it('should reject non-function values', function () {
      _assert2.default.throws(session.session.bind(null, { genid: 'bogus!' }), /genid.*must/);
    });
  ...
  describe('unset option', function () {
    it('should reject unknown values', function () {
      _assert2.default.throws(session.session.bind(null, { unset: 'bogus!' }), /unset.*must/);
    });
    ...
    describe('.regenerate()', function () {
      it('should destroy/replace the previous session', function (done) {
        ...
        (0, _supertest2.default)(server).get('/').expect(shouldSetCookie('connect.sid')).expect(200, function (err, res) {
          if (err) return done(err);
          var id = sid(res);
          (0, _supertest2.default)(server).get('/').set('Cookie', cookie(res)).expect(shouldSetCookie('connect.sid')).expect(200, 'false', function (err, res) {
            if (err) return done(err);
...
_util2.default.inherits(SyncStore, session.Store);
