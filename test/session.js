...
import after_after from "after";
import assert_assert from "assert";
import express_express from "express";
import supertest_request from "supertest";
import cookieparser_cookieParser from "cookie-parser";
...
describe('session()', function(){
  it('should export constructors', function(){
    assert_assert.equal(typeof index_sessionjs.Session, 'function')
    assert_assert.equal(typeof index_sessionjs.Store, 'function')
    assert_assert.equal(typeof index_sessionjs.MemoryStore, 'function')
  })
  ...
  it('should do nothing if req.session exists', function(done){
    ...
    supertest_request(createServer(setup))
    .get('/')
  ...
  it('should error without secret', function(done){
    supertest_request(createServer({ secret: undefined }))
    .get('/')
  ...
  it('should get secret from req.secret', function(done){
    ...
    supertest_request(createServer(setup, { secret: undefined }))
    .get('/')
    .expect(200, '', done)
  })
  ...
  it('should create multiple sessions', function (done) {
    ...
    function check(err) {
      ...
      store.all(function (err, sess) {
        if (err) return done(err)
        assert_assert.equal(Object.keys(sess).length, 2)
        done()
      })
    }
  ...
  it('should update cookie expiration when slow write', function (done) {
    ...
    .expect(200, function (err, res) {
      ...
      setTimeout(function () {
        ...
        .set('Cookie', cookie(res))
        .expect(shouldSetCookie('connect.sid'))
        .expect(function (res) { assert_assert.notEqual(originalExpires, expires(res)); })
        .expect(200, done);
    ...
    it('should have saved session with updated cookie expiration', function (done) {
      ...
      .expect(200, function (err, res) {
        ...
        store.get(id, function (err, sess) {
          if (err) return done(err)
          assert_assert.ok(sess, 'session saved to store')
          var exp = new Date(sess.cookie.expires)
          assert_assert.equal(exp.toUTCString(), expires(res))
          setTimeout(function () {
            supertest_request(server)
            .get('/')
            .set('Cookie', cookie(res))
            .expect(200, function (err, res) {
              ...
              store.get(id, function (err, sess) {
                if (err) return done(err)
                assert_assert.equal(res.text, id)
                assert_assert.ok(sess, 'session still in store')
                assert_assert.notEqual(new Date(sess.cookie.expires).toUTCString(), exp.toUTCString(), 'session cookie expiration updated')
                done()
              })
            })
          }, (1000 - (Date.now() % 1000) + 200))
      ...
      .expect(200, 'session 1', function (err, res) {
        ...
        store.clear(function (err) {
          ...
          .expect(200, 'session 2', function (err, res) {
            if (err) return done(err)
            assert_assert.notEqual(sid(res), val)
            done()
          })
        })
      })
    ...
    it('should not exist in store', function (done) {
      ...
      .expect(200, 'session 1', function (err, res) {
        ...
        setTimeout(function () {
          store.all(function (err, sess) {
            if (err) return done(err)
            assert_assert.equal(Object.keys(sess).length, 0)
            done()
          })
        }, 10)
      })
  ...
  describe('genid option', function(){
    it('should reject non-function values', function(){
      assert_assert.throws(index_sessionjs.bind(null, { genid: 'bogus!' }), /genid.*must/)
    });

    it('should provide default generator', function(done){
      supertest_request(createServer())
      .get('/')
      .expect(shouldSetCookie('connect.sid'))
      .expect(200, done)
    });
    ...
    it('should allow custom function', function(done){
      ...
      supertest_request(createServer({ genid: genid }))
      .get('/')
  ...
  describe('key option', function(){
    ...
    it('should allow overriding', function(done){
      supertest_request(createServer({ key: 'session_id' }))
      .get('/')
  ...
  describe('resave option', function(){
    ...
    it('should pass session touch error', function (done) {
      ...
      server.on('error', function onerror(err) {
        assert_assert.ok(err)
        assert_assert.equal(err.message, 'boom!')
        cb()
      })
  ...
  describe('secret option', function () {
    it('should reject empty arrays', function () {
      assert_assert.throws(createServer.bind(null, { secret: [] }), /secret option array/);
    })
    ...
    describe('when an array', function () {
      ...
      it('should sign cookies with first element', function (done) {
        ...
        supertest_request(server1)
        .get('/')
        .expect(shouldSetCookie('connect.sid'))
        .expect(200, 'bob', function (err, res) {
          if (err) return done(err);
          supertest_request(server2)
          .get('/')
          .set('Cookie', cookie(res))
  ...
  describe('unset option', function () {
    it('should reject unknown values', function(){
      assert_assert.throws(index_sessionjs.bind(null, { unset: 'bogus!' }), /unset.*must/)
    });
  ...
  describe('res.end patch', function () {
    it('should correctly handle res.end/res.write patched prior', function (done) {
      ...
      supertest_request(createServer(setup, null, respond))
      .get('/')
      .expect(200, 'hello, world', done)
    })
    ...
    it('should correctly handle res.end/res.write patched after', function (done) {
      ...
      supertest_request(createServer(null, respond))
      .get('/')
      .expect(200, 'hello, world', done)
    })
  })
      ...
      it('should destroy/replace the previous session', function(done){
        ...
        .expect(200, function (err, res) {
          ...
          .expect(200, 'false', function (err, res) {
            if (err) return done(err)
            assert_assert.notEqual(sid(res), id)
            done();
          });
        });
      })
          ...
          store.get(id, function (err, sess) {
            ...
            setTimeout(function () {
              ...
              .expect(200, function (err, res) {
                ...
                store.get(id, function (err, sess) {
                  if (err) return done(err)
                  assert_assert.notEqual(new Date(sess.cookie.expires).getTime(), exp.getTime())
                  done()
                })
              })
            }, 100)
    ...
    describe('.cookie', function(){
      describe('.*', function(){
        ...
        it('should default to a browser-session length cookie', function(done){
          supertest_request(createServer({ cookie: { path: '/admin' } }))
          .get('/admin')
      ...
      describe('.secure', function(){
        ...
        it('should set cookie when secure', function (done) {
          var cert = fs_fs.readFileSync(__dirname + '/fixtures/server.crt', 'ascii')
          var server = https_https.createServer({
            key: fs_fs.readFileSync(__dirname + '/fixtures/server.key', 'ascii'),
            cert: cert
          })

          server.on('request', app)

          var agent = new https_https.Agent({ca: cert})
          ...
          var req = supertest_request(server).get('/')
          req.agent(agent)
        ...
        it('should not set-cookie when insecure', function(done){
          var server = http_http.createServer(app)
  ...
  describe('cookieParser()', function () {
    ...
    it('should read from req.signedCookies', function(done){
      var app = express_express()
        .use(cookieparser_cookieParser('keyboard cat'))
...
function createServer (options, respond) {
  var fn = respond
  var opts = options
  var server = http_http.createServer()
...
function shouldNotHaveHeader(header) {
  return function (res) {
    assert_assert.ok(!(header.toLowerCase() in res.headers), 'should not have ' + header + ' header')
  };
}
...
function shouldNotSetSessionInStore(store) {
  ...
  return function () {
    assert_assert.ok(count === 0, 'should not set session in store')
  };
}
...
function shouldSetCookie (name) {
  return function (res) {
    ...
    assert_assert.ok(header, 'should have a cookie header')
    assert_assert.equal(data.name, name, 'should set cookie ' + name)
  };
}
...
function shouldSetCookieToExpireIn (name, delta) {
  return function (res) {
    ...
    assert_assert.ok(header, 'should have a cookie header')
    assert_assert.equal(data.name, name, 'should set cookie ' + name)
    assert_assert.ok(('expires' in data), 'should set cookie with attribute Expires')
    assert_assert.ok(('date' in res.headers), 'should have a date header')
    assert_assert.equal((Date.parse(data.expires) - Date.parse(res.headers.date)), delta, 'should set cookie ' + name + ' to expire in ' + delta + ' ms')
  };
}
...
function shouldSetCookieToValue (name, val) {
  return function (res) {
    ...
    assert_assert.ok(header, 'should have a cookie header')
    assert_assert.equal(data.name, name, 'should set cookie ' + name)
    assert_assert.equal(data.value, val, 'should set cookie ' + name + ' to ' + val)
  };
}
...
function shouldSetCookieWithAttribute (name, attrib) {
  return function (res) {
    ...
    assert_assert.ok(header, 'should have a cookie header')
    assert_assert.equal(data.name, name, 'should set cookie ' + name)
    assert_assert.ok((attrib.toLowerCase() in data), 'should set cookie with attribute ' + attrib)
  };
}
...
function shouldSetCookieWithAttributeAndValue (name, attrib, value) {
  return function (res) {
    ...
    assert_assert.equal(data.name, name, 'should set cookie ' + name)
    assert_assert.ok((attrib.toLowerCase() in data), 'should set cookie with attribute ' + attrib)
    assert_assert.equal(data[attrib.toLowerCase()], value, 'should set cookie with attribute ' + attrib + ' set to ' + value)
  };
}
...
function shouldSetCookieWithoutAttribute (name, attrib) {
  return function (res) {
    ...
    assert_assert.ok(header, 'should have a cookie header')
    assert_assert.equal(data.name, name, 'should set cookie ' + name)
    assert_assert.ok(!(attrib.toLowerCase() in data), 'should set cookie without attribute ' + attrib)
  };
}
...
function shouldSetSessionInStore(store) {
  ...
  return function () {
    assert_assert.ok(count === 1, 'should set session in store')
  };
}
...
util_util.inherits(SyncStore, index_sessionjs.Store)
